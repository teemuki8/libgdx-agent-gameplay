...
class GameplayBulletBridgeTest {
    @Test void consecutivePhysicsTicksPreservePartialStanceInsteadOfCollapsingToBoolean() {
        ...
        try (var nativeWorld = new GameplayBulletWorld(8)) {
            ...
            var bridge = new GameplayBulletBridge(nativeWorld, CharacterConfig.defaults(),
                    (context, entity) -> new CharacterIntent(Vec3.ZERO, false, true));
            try (var game = GameWorld.builder(GameplayLimits.defaults(), components)
                ...
                var stance = game.entity(EntityId.of("player")).orElseThrow().component(CharacterStance.TYPE).orElseThrow();
                assertTrue(stance.crouchFraction() > 0 && stance.crouchFraction() < 0.4,
                        "completed domain snapshots must retain the intermediate motor fraction: " + stance);
            }
        }
    }
